...
class QMIX:
    def __init__(self, args):
        ...
        if self.args.load_model:
            if os.path.exists(self.model_dir + '/5_rnn_net_params.pkl'):
                path_rnn = self.model_dir + '/5_rnn_net_params.pkl'
                path_qmix = self.model_dir + '/5_qmix_net_params.pkl'
        ...
        self.eval_parameters = list(
            self.eval_qmix_net.parameters()) + list(self.eval_rnn.parameters())
    ...
    def _get_inputs(self, batch, transition_idx):
        ...
        # 给obs添加上一个动作、agent编号
        # inputs = torch.tensor(obs.reshape((-1, 3, 5, 5)))
        # inputs_next = torch.tensor(obs_next.reshape((-1, 3, 5, 5)))
    ...
    def save_model(self, i, train_step):
        num = str(train_step)
    ...
    def save_final_model(self, i):
        if not os.path.exists(self.model_dir):
            os.makedirs(self.model_dir)
        torch.save(self.eval_qmix_net.state_dict(),
                   self.model_dir + '/' + str(i) + '_qmix_net_params.pkl')
        torch.save(self.eval_rnn.state_dict(),
                   self.model_dir + '/' + str(i) + '_rnn_net_params.pkl')
